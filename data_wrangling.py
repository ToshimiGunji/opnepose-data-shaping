...
def data_wrangling(person_num, gl_pr, down_up, gl_pr_set):
    ...
    for num, file in enumerate(files):
        with open(file) as open_file:
            ...
            # データの上書き、新規作成
            target_file = "./json/shaped/{0}/{1}/{2}/{3}/shaped_{4}.json" \
                .format(person_num, gl_pr, down_up, gl_pr_set, num)
            fw = open(target_file, 'w')
            json.dump(shaped_data, fw, indent=4)
            print(file + "  -->  " + target_file[30:-4])


#         # データの上書き、新規作成
#         fw = open('./shaped_json/0/up/new_{0}.json'.format(num), 'w')
#         json.dump(shaped_data, fw, indent=4
...
for person in person_num:
    for g_p in gl_pr:
        for d_u in down_up:  # ギャラリーデータセットへアクセス
            if g_p == "gallery":
                for gl in gl_set:
                    # print("./json/raw/{0}/{1}/{2}/{3}".format(person, g_p, d_u, gl))
                    data_wrangling(person, g_p, d_u, gl)
            if g_p == "probe":
                for pr in pr_set:
                    # print("./json/raw/{0}/{1}/{2}/{3}/".format(person, g_p, d_u, pr))
                    data_wrangling(person, g_p, d_u, pr)
    if person == 1:
        break

# ファイルごとに操作
# files = natsorted(glob.glob("./json/raw/1/*"))
